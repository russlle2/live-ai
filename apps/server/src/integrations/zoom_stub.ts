...
export async function writeZoomEvent(req: IntegrationWriteRequest): Promise<IntegrationWriteResult> {
  const cfg = connectorEnvFor("zoom");
  let res: IntegrationWriteResult;

  if (!cfg?.endpoint) {
    res = {
      status: "retryable_error",
      externalId: `zoom_cfg_missing_${Date.now()}`,
      message: "ZOOM_WEBHOOK_URL is not configured"
    };
  } else {
    ...
    res = outbound.ok
      ? { status: "ok", externalId: `zoom_${Date.now()}`, message: `Zoom connector dispatched (${outbound.status})` }
      : {
          status: outbound.status >= 500 || outbound.status === 599 ? "retryable_error" : "fatal_error",
          externalId: `zoom_err_${Date.now()}`,
          message: `Zoom connector failed (${outbound.status}): ${outbound.bodyText}`
        };
  }
