...
1. Local loopback coaching/transcription is preferred when configured. Any standard OpenAI key remains server-side; browsers receive only short-lived cloud transcription client secrets.
...
5. WebSockets require one authenticated start within five seconds, admit one audio host plus companions, enforce browser origin/connection/backpressure limits, expire with JWTs, and terminate missing heartbeats.
...
9. Local personal memory may include explicitly qualified review-gated normal/sensitive context, but cannot assert it as biography. Cloud memory remains review-clear/verified. Restricted memory never enters automatic prompts.
...
12. New speech aborts obsolete generation through propagated signals and deadlines. Private-data purge epochs prevent in-flight learning from repopulating deleted state.
