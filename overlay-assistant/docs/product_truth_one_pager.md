...
- one short-lived OpenAI Realtime transcription session per source;
- authenticated laptop audio-host and phone companion pairing;
- immediate cushions, a deterministic provisional line, and a short structured best line with stale-result cancellation;
...
- automatic delivered-suggestion versus verified-owner-speech comparison and cross-session style learning;
...
- automatic three-turn owner-voice enrollment and a fail-closed pinned Hugging Face verifier;
...
Git and Docker image exclusions do not make private resources unavailable. The server automatically loads the server-side OpenAI key, retrieves eligible personal memory, writes transcript/style logs, refreshes authorized Gmail/Drive state, and calls the speaker verifier. The user does not paste keys, transcripts, embeddings, or memory into each session.

Memory eligibility is exact: normal facts must be review-clear; sensitive facts must be owner-verified and review-clear; restricted facts are never automatically retrieved. A stored fact is not necessarily a prompt fact.
...
The repository excludes populated environment files, `data/private`, recordings, voice material, Google exports, and runtime databases from publication. Google OAuth material, cursors, and minimized source cache are application-encrypted. Populated environment files/API keys, personal memory, transcript/style logs, managed auth state, and speaker embeddings are plaintext at rest; app-created private stores have owner-only permissions. Every deployment must use full-disk or encrypted-volume protection and encrypted owner-controlled backups; Git exclusion is not encryption.
