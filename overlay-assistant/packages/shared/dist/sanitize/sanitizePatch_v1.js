...
export function sanitizePatch_v1(input, opts = {}) {
    ...
    if (rawBytes > maxBytes)
        return { ok: false, reason: "payload_too_large", bytes: rawBytes };
    if (!isPlainObject(input))
        return { ok: false, reason: "not_an_object", bytes: rawBytes };
    const droppedPaths = [];
    const out = {};
    // text
    if ("text" in input) {
        const v = input.text;
        if (typeof v === "string")
            out.text = v.slice(0, maxTextChars);
        else
            droppedPaths.push("text");
    }
    // settings allowlist
        ...
        if (isPlainObject(s)) {
            ...
            if ("mirror" in s) {
                ...
                if (typeof v === "boolean")
                    ...
                    droppedPaths.push("settings.mirror");
            }
            if ("opacity" in s) {
                const v = s.opacity;
                if (typeof v === "number")
